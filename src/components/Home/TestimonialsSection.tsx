...
const Testimonial = ({ quote, name, role }: TestimonialProps) => (
  <div className="flex flex-col bg-white/80 rounded-2xl shadow-lg p-7 border border-emerald-100 glassmorphism animate-fade-in hover:scale-105 hover:shadow-xl transition-all duration-300 relative">
    {/* Ornement zellige gold */}
    <svg className="absolute top-4 right-4 w-9 h-9 opacity-25" viewBox="0 0 48 48" fill="none"><path d="M24 1L31.3 16.5L48 18.6L35 30.5L38.5 47L24 38.5L9.5 47L13 30.5L0 18.6L16.7 16.5L24 1Z" fill="#FFD700" fillOpacity="0.23"/></svg>
    <div className="flex-1">
      <div className="h-8 w-8 text-emerald-400 mb-4 opacity-70">
      ...
      <p className="text-emerald-900 italic">{quote}</p>
    </div>
    <div className="mt-6 pt-4 border-t border-emerald-100">
      <p className="text-gold font-semibold">{name}</p>
      <p className="text-emerald-700 text-sm">{role}</p>
    </div>
  </div>
);
...
const TestimonialsSection = () => {
  ...
  return (
    <div className="py-20 bg-gradient-to-r from-emerald-50 via-white/95 to-emerald-100 zellige-pattern">
